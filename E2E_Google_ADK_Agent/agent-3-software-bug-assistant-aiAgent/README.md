...
## 📹 Video Demonstration
[Walkthrough YouTube Video](https://youtu.be/H4oe4h5NGsM)
...
```
                              ...
                              └───────────┬─────────────┘
                                          │
                                    Query │ (e.g., "List open tickets",
                                          │  "Search for CVE-2024-3094")
                                          ▼
                    ┌─────────────────────────────────────────────┐
                    │   Software Bug Assistant Agent (ADK Python) │
                    │   ┌─────────────────────────────────────┐   │
                    │   │  Gemini Model (via Vertex AI or     │   │
                    │   │  Google AI Studio)                  │   │
                    │   │  - Query Understanding              │   │
         ...
         ▼                      ▼        ▼        ▼                        ▼
┌────────────────┐    ┌─────────────────────┐   ┌──────────┐    ┌─────────────────┐
│  MCP Toolbox   │    │   GitHub MCP Server │   │  Google  │    │  StackOverflow  │
│  for Database  │    │                     │   │  Search  │    │   (LangChain)   │
└────────┬───────┘    └──────────┬──────────┘   └────┬─────┘    └────────┬────────┘
         │                       │                    │                   │
         │ SQL Queries           │ API Calls          │ Web Search        │ API Calls
         │ (CRUD, Search)        │                    │                   │
         ▼                       ▼                    ▼                   ▼
┌────────────────┐    ┌─────────────────────┐   ┌──────────┐    ┌─────────────────┐
│  Cloud SQL     │    │   GitHub Repository │   │   Web    │    │  StackExchange  │
│  (PostgreSQL)  │    │   - Issues          │   │  Results │    │   Q&A Database  │
│                │    │   - Pull Requests   │   │          │    │                 │
│  ┌──────────┐  │    │   - Discussions     │   └──────────┘    └─────────────────┘
│  │ Tickets  │  │    └─────────────────────┘
│  │  Table   │  │
│  │          │  │
│  │ + Vector │  │    ┌─────────────────────────────────────────────────────┐
│  │ Embeddings│ │◄───│  Vertex AI Integration (text-embedding-005)         │
│  │   (RAG)  │  │    │  - Generates embeddings for ticket descriptions     │
│  └──────────┘  │    │  - Enables semantic search for similar bugs         │
│                │    └─────────────────────────────────────────────────────┘
└────────────────┘
         │
         │ Results
...
│  - Ticket information (list, details, updates)                             │
│  - External issue references from GitHub                                   │
│  - Search results and solutions from Google/StackOverflow                  │
│  - Recommendations and next steps                                          │
└────────────────────────────────────────────────────────────────────────────┘

Workflow:
1. User submits query via CLI (adk run) or Web UI (adk web)
2. Agent analyzes query and selects appropriate tools
3. Tools fetch data from respective sources (internal DB, GitHub, web, etc.)
4. RAG searches similar tickets using vector embeddings (Cloud SQL + Vertex AI)
5. Agent synthesizes information and generates comprehensive response
6. User receives actionable insights and can perform CRUD operations on tickets
```
...
1. Calling the Gemini API directly using an API key created via Google AI Studio.
2. Calling Gemini models through Vertex AI APIs on Google Cloud.

> **Tip**: If you just want to run the sample locally, an API key from Google AI Studio is the quickest way to get started. If you plan on deploying to Cloud Run, you may want to use Vertex AI.
...
- [☁️ Deploy to Google Cloud](#️-deploy-to-google-cloud)
...
> **Note**: Vector search via `search-tickets` is not yet enabled for local development - see Google Cloud setup below.
...
Once created, you can view your instance in the Cloud Console [here](https://console.cloud.google.com/sql/instances).
...
You should see the tickets table populated with data and embeddings.
...
> **Note**: If you are using Vertex AI instead of AI Studio for Gemini calls, you will need to replace `GOOGLE_API_KEY` with `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, and `GOOGLE_GENAI_USE_VERTEXAI=TRUE` in the last line of the below `gcloud run deploy` command.
>
